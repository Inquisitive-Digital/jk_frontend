...
function AdminAddService() {
  ...
  const editServiceId = location.state?.serviceId ?? null;
  ...
  const loadService = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await serviceAPI.getById(editServiceId);
      if (!res.success || !res.service) throw new Error("Service not found");
      ...
      setFormData({
        ...
        category: s.category || "Business Travel",
        seoTitle: s.meta_title || "",
      ...
      setFaqs(s.faqs || []);
      const imgUrl = s.heroImageUrl || s.heroImage?.url || null;
      if (imgUrl) setHeroImagePreview(getImageUrl(imgUrl));
    } catch (err) {
      setError("Could not load service data. Please go back and try again.");
    } finally {
      setIsFetching(false);
    }
  }, [editServiceId]);

  useEffect(() => { if (isEditing) loadService(); }, [isEditing, loadService]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.subtitle.trim()) fd.append("subtitle", formData.subtitle.trim());
      fd.append("slug", formData.slug.trim());
      fd.append("description", formData.description);
      fd.append("longDescription", formData.longDescription);
      fd.append("category", formData.category);
      if (formData.seoTitle) fd.append("meta_title", formData.seoTitle);
      if (formData.seoDescription) fd.append("meta_description", formData.seoDescription);
      if (formData.script) fd.append("script", formData.script);
      ...
      // Image — only if a new file was picked
      if (heroImage) fd.append("image", heroImage);

      if (isEditing) {
        await serviceAPI.update(editServiceId, fd);
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 mt-6">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. Luxury Airport Transfers"
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    placeholder="e.g. Seamless travel from door to terminal"
                    className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-400 focus:border-transparent outline-none transition-all text-sm"
                  />
                </div>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    Short Description
                    <span className="ml-2 text-xs text-gray-400 font-normal">
                      (shown on the service listing card)
                    </span>
                  </label>
                  <textarea
                    name="description"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    Long Description
                    <span className="ml-2 text-xs text-gray-400 font-normal">
                      (displayed at the top of the service detail page)
                    </span>
                  </label>
                  <textarea
                    ...
                    onWheel={stopTextareaScroll}
                    rows={3}
                    placeholder="A comprehensive introduction to this service…"
                    className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-400 focus:border-transparent outline-none resize-y overflow-y-auto text-sm"
                  />
                </div>
              </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              <h2 className="text-base font-bold text-gray-900 mb-4 flex items-center gap-2">
                <User size={18} className="text-blue-600" /> Category
              </h2>
              <div className="space-y-4">
                <div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    placeholder="Defaults to service title if empty"
                    className="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-400 outline-none text-sm"
                  />
                </div>
                <div>
